from training_evaluation.training_binary import run_training
from training_evaluation.evaluating_binary import run_eval
...
args_eval = {
        'MODEL_FILE_NAME': '',                          
        'RESULT_DIR_NAME': 'binary_test_results',
        'FIND_THRESHOLD': True,
...
try:
    ...
    model_name = args_train["MODEL_NAME_PREFIX"]
    path_to_folder = f'{args_eval["RESULT_DIR_NAME"]}/'
    pattern = os.path.join(path_to_folder, f'*{args_train["MODEL_NAME_PREFIX"]}*')
    ...
    for pred_file in prediction_files:
        ...
        print(f"\nEvaluating predictions for language: {language}")
        ...
        evaluate_subtask_1(gold_file, pred_file, language)
