...
from training_evaluation.model_data_preparation_binary import CustomDataset, get_tokenizer
...
def run_tests(model, testing_loader, find_threshold, evaluation_threshold, only_test, lang, results_list):
    ...
    def get_metrics():
        if targets is None:
            return 0.0
        accuracy = accuracy_score(targets, thresholded_outputs)
        precision = precision_score(targets, thresholded_outputs, pos_label=True)
        recall = recall_score(targets, thresholded_outputs, pos_label=True)
        f1 = f1_score(targets, thresholded_outputs, pos_label=True)
        
        print(f"accuracy = {accuracy}")
        print(f"precision = {precision}")
        print(f"recall = {recall}")
        print(f"f1 score = {f1}")
        results_list.append([lang, current_threshold, accuracy, precision, recall, f1])
        return f1
    ...
    if not only_test:
        print(f"\nBest threshold for {lang}: {best_threshold} with F1 score: {best_f1}")
    return best_outputs, results_list, best_threshold


def create_submission_txt(outputs, df, path):
    ...
    result_df['persuasion_flag'] = [str(bool(output)).lower() for output in outputs]
    
    result_df.to_csv(path, sep='\t', header=None, index=None, columns=['documentID', 'start', 'end', 'persuasion_flag'])
...
def run_eval(args):
    ...
    model = model_class(num_labels=1)
    ...
    for language in languages_to_evaluate:
        ...
        if language_counts.get(language, 'na') == 'na':
            ...
            create_submission_txt(outputs, lang_df, path)

    if not args['ONLY_TEST']:
        df_results = pd.DataFrame(results_list, columns=['language', 'threshold', 'accuracy', 'precision', 'recall', 'f1'])
        df_results = df_results.sort_values(by=['language', 'f1'], ascending=False)
