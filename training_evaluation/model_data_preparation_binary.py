...
class CustomDataset(Dataset):
    def __init__(self, dataframe, tokenizer, max_len, has_targets=True):
        ...
        if has_targets and 'has_technique' in dataframe.columns:
            self.targets = dataframe['has_technique'].values
        else:
            self.targets = None
...
def create_dataset_training(MODEL_NAME: str, train_path: str, test_path: str, TRAINING_LANGUAGES: list[str] = None):
    """
    Function to load and process the data into suitable format for binary classification.
    ...
    print(train_df['has_technique'].value_counts(normalize=True))
    print("\nClass distribution in test set:")
    print(test_df['has_technique'].value_counts(normalize=True))

    train_dataset = CustomDataset(train_df, tokenizer, MAX_LEN, has_targets=True)
    test_dataset = CustomDataset(test_df, tokenizer, MAX_LEN, has_targets=True)

    return train_dataset, test_dataset

def create_dataloader(args, test_df, language, has_targets=True):
    ...
    test_dataset = CustomDataset(filtered_test_df, tokenizer, MAX_LEN, has_targets=has_targets)
